...
-- 5. Crear función para verificar rol HRB APU
...
-- 7. Eliminar políticas antiguas y crear nuevas para tableros
  ...
  CASE 
    WHEN empresa_id IS NOT NULL THEN
      EXISTS (
        ...
        WHERE r.user_id = auth.uid()
          AND r.role = 'HRB APU'
          AND r.empresa_id = calibraciones.empresa_id
      )
    ELSE
      true
  END
